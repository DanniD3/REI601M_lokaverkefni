...
flagList = [
    'round '; %turn rounding on, and optionally provide R s.t. K \in RB_n
    ...
    'a_stop'; %set the end Gaussian, by default "0". 0.5 for standard
    %Gaussian
...
for i=2:size(flags,2)
    if(flags(i)=='-')
        addToMap(ret,flags(start_ind:i-1),possFlags);
        start_ind = i+1;
    end
end
addToMap(ret,flags(start_ind:end),possFlags);
