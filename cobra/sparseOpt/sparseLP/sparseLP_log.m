function solution = sparseLP_log(constraint,params)
% DC programming for solving the sparse LP
% min   ||x||_0 subject to linear constraints
% The l0 norm is approximated by log function.
...
%Initialisation
x = zeros(n,1);
obj_old = sparseLP_log_obj(x,theta);
...
while nbIteration < nbMaxIteration && stop ~= true, 
    ...
    [x,LPsolution] = sparseLP_log_solveSubProblem(subLPproblem,x_bar,theta);
    ...
    switch LPsolution.stat
        ...
        case 1
            ...
            obj_new = sparseLP_log_obj(x,theta);
...
%Solve the linear sub-program to obtain new x
function [x,LPsolution] = sparseLP_log_solveSubProblem(subLPproblem,x_bar,theta)
    
    n = length(x_bar);

    % Define objective - variable (x,t)
    subLPproblem.obj = [-x_bar;(theta/log(1+theta))*ones(n,1)];
...
function obj = sparseLP_log_obj(x,theta)
    n = length(x);
