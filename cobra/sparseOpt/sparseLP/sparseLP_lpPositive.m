function solution = sparseLP_lpPositive(constraint,params)
% DC programming for solving the sparse LP
% min   ||x||_0 subject to linear constraints
% The l0 norm is approximated by  l_p norm with 0<p<1.
...
%Initialisation
x = zeros(n,1);
obj_old = sparseLP_lpPositive_obj(x,epsilonP,p);
...
while nbIteration < nbMaxIteration && stop ~= true, 
    ...
    [x,LPsolution] = sparseLP_lpPositive_solveSubProblem(subLPproblem,x_bar,epsilonP,p);
    ...
    switch LPsolution.stat
        ...
        case 1
            ...
            obj_new = sparseLP_lpPositive_obj(x,epsilonP,p);
...
function [x,LPsolution] = sparseLP_lpPositive_solveSubProblem(subLPproblem,x_bar,epsilonP,p)
    
    n = length(x_bar);

    % Define objective - variable (x,t)
    subLPproblem.obj = [-x_bar;(power(epsilonP,1/(p-1))/p)*ones(n,1)];
...
function obj = sparseLP_lpPositive_obj(x,epsilonP,p)
    n = length(x);
